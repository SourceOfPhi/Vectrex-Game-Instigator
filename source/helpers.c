...
/*
unsigned int getDistance(const struct vector_t* const a, const struct vector_t* const b)
{
	unsigned int ax = (unsigned int) ((unsigned int) a->x + 120U);
	unsigned int bx = (unsigned int) ((unsigned int) b->x + 120U);
...
/*
int getSideOfLine(struct vector_t *A, struct vector_t *B, struct vector_t *C)
{
    // checks on wicht side of the line AB point C lies
    // > 0 left
    // < 0 right
    // == 0 on the line
...
int isPointInTriangle(struct vector_t *A, struct vector_t *B, struct vector_t *C, struct vector_t *PT)
{
    return (getSideOfLine(A, B, PT) < 0 && getSideOfLine(B, C, PT) < 0 && getSideOfLine(C, A, PT) < 0);
}

int intpoint_inside_trigon(struct vector_t *s, struct vector_t *a, struct vector_t *b, struct vector_t *c)
{
